...
#define AMPLITUDE       1400          // Depending on your audio source level, you may need to alter this value. Can be used as a 'sensitivity' control.
...
#define NOISE           175           // Used as a crude noise filter, values below this are ignored
...
void loop() {
  ...
  for (int i = 2; i < (SAMPLES/2); i++){       // Don't use sample 0 and only first SAMPLES/2 are usable. Each array element represents a frequency bin and its value the amplitude.
    if (vReal[i] > NOISE) {                    // Add a crude noise filter
      ...
      if (i>7   && i<=10 ) bandValues[6]  += (int)vReal[i];
      if (i>10  && i<=12 ) bandValues[7]  += (int)vReal[i];
      if (i>12  && i<=16 ) bandValues[8]  += (int)vReal[i];
      if (i>16  && i<=21 ) bandValues[9]  += (int)vReal[i];
      if (i>21  && i<=27 ) bandValues[10] += (int)vReal[i];
      if (i>27  && i<=36 ) bandValues[11] += (int)vReal[i];
      if (i>36  && i<=47 ) bandValues[12] += (int)vReal[i];
      if (i>47  && i<=61 ) bandValues[13] += (int)vReal[i];
      if (i>61  && i<=79 ) bandValues[14] += (int)vReal[i];
      if (i>79  && i<=103 ) bandValues[15] += (int)vReal[i];
      if (i>103  && i<=134 ) bandValues[16] += (int)vReal[i];
      if (i>134  && i<=174 ) bandValues[17] += (int)vReal[i];
      if (i>174  && i<=226 ) bandValues[18] += (int)vReal[i];
      if (i>290            ) bandValues[19] += (int)vReal[i];
     ...
     /*   //19 bands, 12kHz top band  //512 bit   30000 freqency
      ...
      if (i>134  && i<=174 ) bandValues[17] += (int)vReal[i];
      if (i>174  && i<=226 ) bandValues[18] += (int)vReal[i];
      if (i>295            ) bandValues[19] += (int)vReal[i];
      */
